Drupal.behaviors.digitalCollectionsBehavoir = function (context) {
  ...
  arr = window.document.URL.split('/');
  term = arr[2 + Drupal.settings.basePath.split('/').length];
...
function new_fake_link(i, dom, url) {
  ...
  label = $('div', dom).clone();
  $(dom).empty().append(label).append('<a href="'+ Drupal.settings.basePath + url + '/' + value + '">' + value +'</a>');
}
